...
# добавить аннотацию типов, я бы даже сказал, что со временем это будет Критично, так как очень много операций с таблицами, списками, числами
# и при масштабировании через функции голова кругом пойдет от обилия переменных без явного указания типа
...
# Можно лучше
# если данные величины - константы, то перевести имена в верхний регистр
pac_first_yind = 7550
pac_last_yind = -2
atl_first_yind = 7350
atl_last_yind = -1
...
def grid_selector(pcf, var, extent, pac_patch=False):
    ...
    # КРИТИЧНО
    # можно получить несколько ошибок (KeyError, AttributeError), если не проверять наличие var in pcf
    ...
    if pac_patch:  # Pacific patch selection
        ...
        # в зависимости от перемнной var
        if var in t_vars:
...
# Можно лучше
# atl_dataset и pac_dataset создаются схожим образом, лучше вынести код в отдельную функцию
...
# Критично
# по поводу пути выше расписал + лучше использовать os.path.join
whole_dataset.to_netcdf(f'{target_path}/{target_name}')

# TODO: Some visualization?
# Можно лучше
# использовать модули plt/sns
